 ...
 */
function $collectReferencedFootnoteIds(nodes: readonly LexicalNode[]): string[] {
  const cited: string[] = [];
...
function $serializeSubtree(node: LexicalNode): SerializedLexicalNode {
  ...
  if ($isElementNode(node)) {
    (json as SerializedElementNode).children =
      node.getChildren().map($serializeSubtree);
  }
  return json;
}
...
const $htmlWithFootnotes: ExportMimeTypeFunction = (selection, next) => {
  ...
  const footnoteIds = $collectReferencedFootnoteIds(selection.getNodes()).filter(
    footnoteId => $getDefinitionSlot(section, footnoteId) !== null,
  );
  ...
  return (
    html + $buildFootnoteSectionDOM($getEditor(), section, footnoteIds).outerHTML
  );
};
 ...
 */
export function $remapCollidingFootnoteIds(nodes: readonly LexicalNode[]): void {
